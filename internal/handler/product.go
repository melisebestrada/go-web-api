...
func (ph *ProductHandler) GetProductById() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			web.SendResponse(w, err.Error(), nil, true, http.StatusNotFound)
			return
		}

		web.SendResponse(w, "product found", product, false, http.StatusOK)
	}
}
...
func (ph *ProductHandler) DeleteProduct() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		deletedProduct, err := ph.service.DeleteProduct(idProduct)
		if err != nil {
			web.SendResponse(w, err.Error(), nil, true, http.StatusBadRequest)
			return
		}

		web.SendResponse(w, "product deleted", deletedProduct, false, http.StatusOK)
	}
}
